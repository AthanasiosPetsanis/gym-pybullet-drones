...
def run(multiagent=DEFAULT_MA, output_folder=DEFAULT_OUTPUT_FOLDER, gui=DEFAULT_GUI, plot=True, colab=DEFAULT_COLAB, record_video=DEFAULT_RECORD_VIDEO, local=True):
    ...
    if not multiagent:
        train_env = make_vec_env(VisionAviary, 
                                 env_kwargs=dict(obs=DEFAULT_OBS, act=DEFAULT_ACT, ctrl_freq=24),
                                 n_envs=1
                                 )
        eval_env = VisionAviary(obs=DEFAULT_OBS, act=DEFAULT_ACT, ctrl_freq=24)
    else:
    ...
    if not multiagent: 
        test_env = VisionAviary(gui=gui,
                               obs=DEFAULT_OBS,
                               act=DEFAULT_ACT,
                               record=record_video)
        test_env_nogui = VisionAviary(obs=DEFAULT_OBS, act=DEFAULT_ACT)
    else:
